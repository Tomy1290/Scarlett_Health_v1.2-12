...
export default function SettingsScreen() {
  ...
  async function saveCustomReminder() {
    ...
    // Schedule next occurrence one-time (prevents immediate fire), and store meta
    ...
    if (notifId) {
      state.addReminder({ 
        ...
        time: `${timeData.hour.toString().padStart(2,'0')}:${timeData.minute.toString().padStart(2,'0')}`,
        enabled: true 
      });
  ...
  async function seedDefaults() {
    ...
    for (const def of defaults) {
      const notifId = await scheduleDailyNext(
        def.id, 
        def.title, 
        def.body, 
        def.hour, 
  ...
  async function updateTime(id: string, newTime: string) {
    ...
    if (r.enabled) {
      ...
      const newNotifId = await scheduleDailyNext(
        id, 
        title, 
        state.language==='de'?'Zeit für eine Aktion':(state.language==='pl'?'Czas na działanie':'Time for an action'), 
        timeData.hour, 
