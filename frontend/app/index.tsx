...
import UnlockModal, { UnlockItem } from "../src/components/UnlockModal";
...
export default function Home() {
  ...
  useEffect(() => {
    if (level > prevLevelRef.current) {
      setCelebrationText(language==='de' ? `Level ${level}` : (language==='pl'?`Poziom ${level}`:`Level ${level}`));
      setShowCelebration(true);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      // New unlocks
      try {
        const newly = FEATURES.filter(f => f.level > prevLevelRef.current && f.level <= level).map(f => ({ id: f.id, title: f.title(language as any), description: f.description(language as any) }));
        if (newly.length > 0) { setUnlockItems(newly); setUnlockVisible(true); }
      } catch {}
  ...
  const bmiFeatureUnlocked = level >= 60;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}> 
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 10 }}>
            {/* Morning */}
            <TouchableOpacity onPress={() => { togglePill(currentDate, 'morning'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }} style={[styles.toggle, { borderColor: colors.primary, backgroundColor: day.pills.morning ? colors.primary : 'transparent' }]}>
              <Ionicons name="sunny" size={18} color={day.pills.morning ? '#fff' : colors.primary} />
              <Text style={{ color: day.pills.morning ? '#fff' : colors.text, marginLeft: 6 }}>{t('Morgens', 'Morning', 'Rano')}</Text>
            </TouchableOpacity>
            {/* Evening */}
            <TouchableOpacity onPress={() => { togglePill(currentDate, 'evening'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }} style={[styles.toggle, { borderColor: colors.primary, backgroundColor: day.pills.evening ? colors.primary : 'transparent' }]}>
              <Ionicons name="moon" size={18} color={day.pills.evening ? '#fff' : colors.primary} />
              <Text style={{ color: day.pills.evening ? '#fff' : colors.text, marginLeft: 6 }}>{t('Abends', 'Evening', 'Wieczorem')}</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}> 
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            ...
            <TouchableOpacity onPress={() => { toggleFlag(currentDate, 'gingerGarlicTea'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }} style={[styles.chip, { borderColor: colors.primary, backgroundColor: day.drinks.gingerGarlicTea ? colors.primary : 'transparent' }]} accessibilityLabel={t('Ingwer-Knoblauch-Tee', 'Ginger & garlic tea', 'Herbata imbir-czosnek')} >
              <Text style={{ color: day.drinks.gingerGarlicTea ? '#fff' : colors.text }}>{t('Ingwer-Knoblauch-Tee', 'Ginger & garlic tea', 'Herbata imbir-czosnkowa')}</Text>
            </TouchableOpacity>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}> 
          ...
          {/* BMI bar gated by level */}
          {bmiFeatureUnlocked && typeof bmiVal === 'number' ? (
      ...
      <UnlockModal visible={unlockVisible} onClose={() => setUnlockVisible(false)} items={unlockItems} colors={colors} />
    </SafeAreaView>
  );
}
